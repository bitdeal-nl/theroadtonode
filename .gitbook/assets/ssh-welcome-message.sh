...
# set datadir
bitcoin_dir="/home/pi/.bitcoin"
lnd_dir="/home/pi/.lnd"

# set blockexplorerip
blockexplorerip="192.168.2.1"
...
if [ "${gitupdate}" -eq "1" ]; then
  # Calls to github
  btcgit=$(curl -s https://api.github.com/repos/bitcoin/bitcoin/tags | jq -r '.[0].name')
  ...
  electrsgit=$(curl -s https://api.github.com/repos/chris-belcher/electrum-personal-server/releases/latest | grep -oP '"tag_name": "eps-v\K(.*)(?=")')
  #electrsgit=$(curl -s https://github.com/chris-belcher/electrum-personal-server/tags | jq -r '.[0].name')
...
#electrs
electrs_running=$(systemctl is-active eps)
...
if [ -z "${electrs_running##*inactive*}" ]; then
  ...
  electrs_running="up"
  #electrspi=$(echo '{"jsonrpc": "2.0", "method": "server.version", "id": 0}' | netcat 127.0.0.1 50001 -q 1 | jq -r '.result[0]' | awk '{print "v"$2}')
  electrspi=$(sudo -u bitcoin /home/pi/.local/bin/electrum-personal-server --version | sed -n 's/^.* //p')
...
#btcrpcexplorer
btcrpcexplorer_running=$(systemctl is-active btc-rpc-explorer)
...
if [ -z "${btcrpcexplorer_running##*inactive*}" ]; then
  ...
  btcrpcexplorerpi=$(curl -s http://${blockexplorerip}:3002/tools | grep -oP '(?<=version: )[^</]*' | awk '{print "v"$1}' )
...
#render
printf "
${color_yellow}Road2Node:${color_grey} Bitcoin Core, LND, EPS and Block Explorer
${color_yellow}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
"Electrum (EPS)" "${electrs_running}" "Block Explorer" "${btcrpcexplorer_running}" \
